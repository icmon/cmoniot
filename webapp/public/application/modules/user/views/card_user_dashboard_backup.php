...
async function updateStatus(userId, newStatus) {
    try {
        ...
        const response = await fetch(
            `<?php echo $apipostupdateprofile;?>?uid=${userId}&active_status=${newStatus}`, { // ใช้ตัวแปร PHP สำหรับ URL
                method: 'POST', // หรือ GET หาก API ของคุณกำหนดไว้เช่นนั้น แต่ POST/PUT เป็นมาตรฐานสำหรับการอัปเดต
                headers: {
                    // 'Content-Type': 'application/json', // ไม่จำเป็นถ้าส่งข้อมูลผ่าน URL parameters
                    // หากระบบของคุณใช้ CSRF token ให้เพิ่ม header ที่นี่
                    // 'X-CSRF-TOKEN': 'your_csrf_token_value'
                }
                // body: JSON.stringify({ uid: userId, active_status: newStatus }) // ใช้ส่วนนี้หาก API คาดหวัง JSON body
            });
                ...
                render: function(data_uid, type, row, meta) {
                    ...
                    return `
                        <div class="action-buttons">
                            <a href="<?php echo base_url('settings/schedule');?>/detail?uid=${data_uid}" class="btn btn-sm btn-info me-1" title="<?php echo $this->lang->line('c_detail');?>"><?php echo $this->lang->line('c_detail');?></a>
                            <a href="<?php echo base_url('settings/schedule');?>/edit?uid=${data_uid}" class="btn btn-sm btn-warning me-1" title="<?php echo $this->lang->line('c_edit');?>"><?php echo $this->lang->line('c_edit');?></a>
