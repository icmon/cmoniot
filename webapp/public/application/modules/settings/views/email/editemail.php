...
// รองรับกรณีรับ email_id กับ id
$email_id = $input['email_id'] ?? '';
if ($email_id == "") {
    $email_id = $input['id'] ?? '';
}

$api_url = $this->config->item('api_url') . 'settings/listemail?email_id=' . $email_id;
$api_updatealarm = $this->config->item('api_url') . 'settings/updateemail';
$home_url = base_url('settings/email');
$detail_url = base_url('alarm/email/detail?email_id=' . $email_id);
?>
<!-- CSS -->
                ...
                <form id="alarmForm">
                    <div class="card">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h3 class="card-title mb-0">Edit email</h3>
                            <div>
    ...
    fetch(api_url, {
        ...
        .then(resp => {
            if (resp.payload && resp.payload.data && resp.payload.data.length > 0) {
                ...
                document.getElementById('email_name').value = data.email_name || '';
