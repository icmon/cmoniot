...
$api_url = $this->config->item('api_url') . 'settings/schedule?schedule_id=' . $schedule_id;
$api_createschedule= $this->config->item('api_url') . 'settings/createschedule';
...
$detail_url = base_url('settings/schedule/detail?schedule_id=' . $schedule_id);
?>
...
<script>
var schedule_id = <?php echo json_encode($input['schedule_id']); ?>;
var apiUrl = <?php echo json_encode($detail_url); ?>;
var apiTypeAll = <?php echo json_encode($api_typeall); ?>;
var apiUpdateSchedule = <?php echo json_encode($api_updateschedule); ?>;
var api_createschedule = <?php echo json_encode($api_createschedule); ?>;
    ...
    fetch(api_createschedule, {
        ...
        .then(resp => {
            ...
            if (resp.status === 'success' || resp.code === 201 || resp.statusCode === 201 || resp.code ===
                ...
                msgBox.innerHTML = '<div class="alert alert-success">Add data successful!</div>';
                Swal.fire({
                    icon: 'success',
                    ...
                    window.location.href = "<?php echo base_url('settings/schedule'); ?>";
                });
            } else {

                msgBox.innerHTML = '<div class="alert alert-danger">Add data failed: ' + (resp.message ||
                    'Unknown error') + '</div>';
                Swal.fire({
                    icon: 'error',
                    title: 'Unknown error!',
        ...
        .catch(error => {
            ...
            document.getElementById('responseMessage').innerHTML =
                '<div class="alert alert-danger">Error Add data: ' + error + '</div>';
            Swal.fire({
                icon: 'error',
