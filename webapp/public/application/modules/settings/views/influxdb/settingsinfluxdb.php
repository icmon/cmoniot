...
<?php $this->load->view('settings/influxdb/pagewrapper'); ?>
<div class="page-body">
    <?php  /*****body**********/  ?>
    <div class="card-body">
        <div class="card">
...
async function updateinfluxdbstatus(influxdb_id, newStatus) {
    try {
        // Swal.fire({
        ...
        var apiUrl = `${baseApiUrl}settings/updateinfluxdbstatus`;
        ...
        // เคลียร์ cache
        var apiUrldeletecache = `${baseApiUrl}settings/listinfluxdbpage?deletecache=1`;
        ...
        if (data.code === 200 || data.success === true) {
            ...
            //     text: '<?php echo $this->lang->line('t2_please_Unabletoupdateinfluxdbstatus'); ?>',
            //     timer: 1500,
            //     timerProgressBar: true,
            //     didOpen: () => {
            //         const confirmButton = Swal.getConfirmButton();
...
async function fetchData(page = 1, pageSize = 10, search = '') {
    ...
    var baseApiUrl = <?php echo json_encode($this->config->item('api_url')); ?>;
    var apiUrl =
        `${baseApiUrl}settings/listinfluxdbpage?page=${page}&pageSize=${pageSize}&keyword=${encodeURIComponent(search)}&influxdb_id=${influxdb_id}`;
    var response = await fetch(apiUrl, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${bearerToken}`,
...
$(document).ready(function() {
                ...
                render: function(data, type, row, meta) {
                    ...
                    return `<div class="form-check form-switch d-flex justify-content-center align-items-center m-0 p-0" style="height: 1.5rem;">
                        <input class="form-check-input status-switch" type="checkbox" role="switch"
                            id="switch-${row.influxdb_id}" data-influxdb_id="${row.influxdb_id}" ${isChecked}
                            aria-label="User status switch for ${row.influxdb_id}">
                    </div>`;
                }
            },
            {
                ...
                render: function(data, type, row, meta) {
                    // var editBtn =
                    //     `<a href="<?php echo base_url('settings/influxdb');?>/edit?influxdb_id=${row.influxdb_id}" class="btn btn-sm btn-warning me-1"><?php echo $this->lang->line('c_edit');?></a>`;
                    // var deleteBtn =
                    //     `<a href="javascript:void(0);" class="btn btn-sm btn-danger" onclick="deleteRow('${row.influxdb_id}')"><?php echo $this->lang->line('c_delete');?></a>`;
    ...
    $('#example').on('change', '.status-switch', async function() {
        const influxdb_id = $(this).data('influxdb_id');
        const newStatus = $(this).is(':checked') ? 1 : 0;
        const success = await updateinfluxdbstatus(influxdb_id, newStatus);
    ...
    window.deleteRow = function(influxdb_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${baseApiUrl}settings/deleteinfluxdb?influxdb_id=${influxdb_id}`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${bearerToken}`,
                            'Content-Type': 'application/json'
