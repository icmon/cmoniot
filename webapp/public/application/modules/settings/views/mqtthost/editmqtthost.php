...
// รองรับกรณีรับ id กับ id
$id = $input['id'] ?? '';
if ($id == "") {
    $id = $input['id'] ?? '';
}

$api_url = $this->config->item('api_url') . 'settings/listmqtthost?id=' . $id;
$api_updatealarm = $this->config->item('api_url') . 'settings/updatemqtthost';
$home_url = base_url('settings/mqtthost');
$detail_url = base_url('alarm/mqtthost/detail?id=' . $id);
?>
<!-- CSS -->
                ...
                <form id="alarmForm">
                    <div class="card">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h3 class="card-title mb-0">Edit Mqtthost</h3>
                            <div>
...
function validateDevicealarmForm() {
    ...
    let mqtthostName = document.getElementById('hostname').value.trim();
    if (mqtthostName === '') {
        valid = false;
        messages.push('Please enter your mqtthost name');
    }
