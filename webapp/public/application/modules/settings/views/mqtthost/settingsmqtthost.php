...
async function updatemqtthoststatus(id, newStatus) {
    try {
        // Swal.fire({
        ...
        var apiUrl = `${baseApiUrl}settings/updatemqtthoststatus`;
        ...
        // เคลียร์ cache
        var apiUrldeletecache = `${baseApiUrl}settings/listmqtthost?deletecache=1`;
        ...
        if (data.code === 200 || data.success === true) {
            ...
            //     text: '<?php echo $this->lang->line('t2_please_Unabletoupdatemqtthoststatus'); ?>',
            //     timer: 1500,
            //     timerProgressBar: true,
            //     didOpen: () => {
            //         const confirmButton = Swal.getConfirmButton();
...
async function fetchData(page = 1, pageSize = 10, search = '') {
    ...
    var baseApiUrl = <?php echo json_encode($this->config->item('api_url')); ?>;
    var apiUrl =
        `${baseApiUrl}settings/listmqtthost?page=${page}&pageSize=${pageSize}&keyword=${encodeURIComponent(search)}&id=${id}`;
    var response = await fetch(apiUrl, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${bearerToken}`,
...
$(document).ready(function() {
                ...
                render: function(data, type, row, meta) {
                    ...
                    return `<div class="form-check form-switch d-flex justify-content-center align-items-center m-0 p-0" style="height: 1.5rem;">
                        <input class="form-check-input status-switch" type="checkbox" role="switch"
                            id="switch-${row.id}" data-id="${row.id}" ${isChecked}
                            aria-label="User status switch for ${row.id}">
                    </div>`;
                }
            },
            {
                ...
                render: function(data, type, row, meta) {
                    // var editBtn =
                    //     `<a href="<?php echo base_url('settings/mqtthost');?>/edit?id=${row.id}" class="btn btn-sm btn-warning me-1"><?php echo $this->lang->line('c_edit');?></a>`;
                    // var deleteBtn =
                    //     `<a href="javascript:void(0);" class="btn btn-sm btn-danger" onclick="deleteRow('${row.id}')"><?php echo $this->lang->line('c_delete');?></a>`;
    ...
    $('#example').on('change', '.status-switch', async function() {
        ...
        const success = await updatemqtthoststatus(id, newStatus);
    ...
    window.deleteRow = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${baseApiUrl}settings/deletemqtthost?id=${id}`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${bearerToken}`,
                            'Content-Type': 'application/json'
