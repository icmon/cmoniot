...
<?php $this->load->view('settings/sms/pagewrapper'); ?>
<div class="page-body">
    <?php  /*****body**********/  ?>
    <div class="card-body">
        <div class="card">
...
async function updatesmsstatus(sms_id, newStatus) {
    try {
        // Swal.fire({
        ...
        var apiUrl = `${baseApiUrl}settings/updatesmsstatus`;
        ...
        // เคลียร์ cache
        var apiUrldeletecache = `${baseApiUrl}settings/listsmspage?deletecache=1`;
...
async function fetchData(page = 1, pageSize = 10, search = '') {
    ...
    var baseApiUrl = <?php echo json_encode($this->config->item('api_url')); ?>;
    var apiUrl =
        `${baseApiUrl}settings/listsmspage?page=${page}&pageSize=${pageSize}&keyword=${encodeURIComponent(search)}&sms_id=${sms_id}`;
    var response = await fetch(apiUrl, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${bearerToken}`,
...
$(document).ready(function() {
                ...
                render: function(data, type, row, meta) {
                    ...
                    return `<div class="form-check form-switch d-flex justify-content-center align-items-center m-0 p-0" style="height: 1.5rem;">
                        <input class="form-check-input status-switch" type="checkbox" role="switch"
                            id="switch-${row.sms_id}" data-sms_id="${row.sms_id}" ${isChecked}
                            aria-label="User status switch for ${row.sms_id}">
                    </div>`;
                }
            },
            {
                ...
                render: function(data, type, row, meta) {
                    // var editBtn =
                    //     `<a href="<?php echo base_url('settings/sms');?>/edit?sms_id=${row.sms_id}" class="btn btn-sm btn-warning me-1"><?php echo $this->lang->line('c_edit');?></a>`;
                    // var deleteBtn =
                    //     `<a href="javascript:void(0);" class="btn btn-sm btn-danger" onclick="deleteRow('${row.sms_id}')"><?php echo $this->lang->line('c_delete');?></a>`;
    ...
    $('#example').on('change', '.status-switch', async function() {
        const sms_id = $(this).data('sms_id');
        const newStatus = $(this).is(':checked') ? 1 : 0;
        const success = await updatesmsstatus(sms_id, newStatus);
    ...
    window.deleteRow = function(sms_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${baseApiUrl}settings/deletesms?sms_id=${sms_id}`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${bearerToken}`,
                            'Content-Type': 'application/json'
