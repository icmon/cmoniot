...
<?php $this->load->view('settings/nodered/pagewrapper'); ?>
<div class="page-body">
    <?php  /*****body**********/  ?>
    <div class="card-body">
        <div class="card">
...
$role_id_api = @$_SESSION['role_id_api'];   
 ...
 ?>
<script>
const role_id_api = <?php echo json_encode($role_id_api); ?>;
const api_url = <?php echo json_encode($api_url); ?>;
const nodered_id = <?php echo json_encode($nodered_id); ?>;
const token = <?php echo json_encode($token); ?>;


async function updatenoderedstatus(nodered_id, newStatus) {
    try {
        // Swal.fire({
        ...
        var apiUrl = `${baseApiUrl}settings/updatenoderedstatus`;
        ...
        var apiUrldeletecache = `${baseApiUrl}settings/listnoderedpaginate?deletecache=1`;
        ...
        if (data.code === 200 || data.success === true) {
            ...
            //     text: '<?php echo $this->lang->line('t2_please_Unabletoupdatenoderedstatus'); ?>',
            //     timer: 1500,
            //     timerProgressBar: true,
            //     didOpen: () => {
            //         const confirmButton = Swal.getConfirmButton();
...
async function fetchData(page = 1, pageSize = 10, search = '') {
    ...
    var baseApiUrl = <?php echo json_encode($this->config->item('api_url')); ?>;
    var apiUrl =
        `${baseApiUrl}settings/listnoderedpaginate?page=${page}&pageSize=${pageSize}&keyword=${encodeURIComponent(search)}&nodered_id=${nodered_id}`;
    var response = await fetch(apiUrl, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${bearerToken}`,
...
$(document).ready(function() {
                ...
                render: function(data, type, row, meta) {
                    ...
                    return `<div class="form-check form-switch d-flex justify-content-center align-items-center m-0 p-0" style="height: 1.5rem;">
                        <input class="form-check-input status-switch" type="checkbox" role="switch"
                            id="switch-${row.nodered_id}" data-nodered_id="${row.nodered_id}" ${isChecked}
                            aria-label="User status switch for ${row.nodered_id}">
                    </div>`;
                }
            },
            {
                ...
                render: function(data, type, row, meta) {
                    // var editBtn =
                    //     `<a href="<?php echo base_url('settings/nodered');?>/edit?nodered_id=${row.nodered_id}" class="btn btn-sm btn-warning me-1"><?php echo $this->lang->line('c_edit');?></a>`;
                    // var deleteBtn =
                    //     `<a href="javascript:void(0);" class="btn btn-sm btn-danger" onclick="deleteRow('${row.nodered_id}')"><?php echo $this->lang->line('c_delete');?></a>`;
    ...
    $('#example').on('change', '.status-switch', async function() {
        const nodered_id = $(this).data('nodered_id');
        const newStatus = $(this).is(':checked') ? 1 : 0;
        const success = await updatenoderedstatus(nodered_id, newStatus);
    ...
    window.deleteRow = function(nodered_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${baseApiUrl}settings/deletenodered?nodered_id=${nodered_id}`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${bearerToken}`,
                            'Content-Type': 'application/json'
