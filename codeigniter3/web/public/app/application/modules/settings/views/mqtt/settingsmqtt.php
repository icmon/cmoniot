            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    ...
                    if (row.device_count == 0) {
                        return `<a href="<?php echo base_url('settings/device/devicesadd');?>?bucket=${row.bucket}&mqtt_id=${row.mqtt_id}" class="btn btn-sm btn-info me-1">Add Device</a>`;
                    } else if (row.status == 1) {
                        return ` <a href="<?php echo base_url('settings/device');?>?bucket=${row.bucket}&mqtt_id=${row.mqtt_id}" class="btn btn-sm btn-primary me-1"> Detail (${row.device_count}) </a>`;
                    } else {
                        return `-`;
                    }
                }
            },
            {
                ...
                render: function(data, type, row, meta) {
                    ...
                    if (row.device_count == 0) {
                        ...
                        return `<a href="<?php echo base_url('settings/device/deviceactive');?>?bucket=${row.bucket}&mqtt_id=${row.mqtt_id}" class="btn btn-sm btn-success me-1">Control (${row.device_count})</a>`;
                    } else {
                        return `-`;
                    }
                }
