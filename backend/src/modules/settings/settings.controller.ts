...
/********create_deviceactionuser**********/ 
                               ...
                               // console.log(`2-devicecontrol=>`); console.info(devicecontrol);
                                ...
                                await this.settingsService.create_scheduleprocesslog(createset);
                                              // sendEmail scheduleproces
                                                if(message_mqtt_control==1){
                                                    var subject_event:any='On';
                                                }else{
                                                    var subject_event:any='Off';
                                                }  
                                                var subject: any = 'Schedule process '+schedule_name+' start '+schedule_event_start+'  day '+today_name;
                                                var content:any='Schedule process '+schedule_name+' start '+schedule_event_start+'  day '+today_name+'  event '+subject_event+'  date '+date_now+'  time '+time_now+' device_id: '+device_id;
                                                var log_alarm_log :any='';
                                                    for (const [k, v] of Object.entries(useractive)) {
                                                        var toemail:any = useractive[k].email;
                                                        var mobile_number:any = useractive[k].mobile_number;
                                                        var lineid:any = useractive[k].lineid;
                                                        var user_arr:any= {email:toemail,mobile:mobile_number,lineid:lineid};
                                                        useractive_arr.push(user_arr); 
                                                          var ResultDatasendEmail: any = await this.settingsService.sendEmail(toemail, subject,content);
                                                    }
                                                // sendEmail
 ...
 // updatealarmstatus
  ...
  async get_alarm_control(alarm_action_id_mas: any, device_id_mas: any) {
      ...
      var contents:any=' normal';
      var alarmStatusSet: number=Number(0);
      var validate_count: number = 3;
      var filter_alarm :any= {
          alarm_action_id: alarm_action_id_mas,
          device_id: device_id_mas
      };
      var device_data_control: any = await this.settingsService.device_list_paginate_alarm_device_event_control(filter_alarm);
      for (const [key, val] of Object.entries(device_data_control)) {
          var deviceData: any = device_data_control[key];
          var alarm_action_id: number = Number(deviceData.alarm_action_id);
          var device_id: number = Number(deviceData.device_id);
          var type_id: number = Number(deviceData.type_id);
          var event: any = deviceData.event;
          var status_warning: any = deviceData.status_warning;
          var recovery_warning: any = deviceData.recovery_warning;
          var status_alert: any = deviceData.status_alert;
          var recovery_alert: any = deviceData.recovery_alert;
          var time_life: any = deviceData.time_life;
          var alarm_type_id: number = type_id;
          var mqtt_name: any = deviceData.mqtt_name;
          var device_name: any = deviceData.device_name;
          var mqtt_control_on:number= Number(deviceData.mqtt_control_on);
          var mqtt_control_off:number= Number(deviceData.mqtt_control_off);
          var action_name: any = deviceData.action_name;
          var mqtt_data_value: any = deviceData.mqtt_data_value;
          var mqtt_data_control: any = deviceData.mqtt_data_control;
          var configdata: any = deviceData.configdata;
          var mqtt_status_data_name: any = deviceData.mqtt_status_data_name;
          var alarmTypeId: number = Number(type_id);
          var statusAlert: any = deviceData.status_alert;
          var statusWarning: any = deviceData.status_warning;
          var recoveryWarning: any = deviceData.recovery_warning;
          var recoveryAlert: any = deviceData.recovery_alert;
          var mqttName: any = deviceData.mqtt_name;
          var deviceName: any = deviceData.device_name;
          var alarmActionName: any = deviceData.action_name;
          var mqttControlOn:number= mqtt_control_on
          var mqttControlOff:number= mqtt_control_off;
          var events : any = deviceData.event;
          // การจัดการข้อมูล MQTT
          var mqttrs: any = await this.mqttService.getdevicedataAll(mqtt_data_value);
          var mqttData = mqttrs['data'];
          var configObj = JSON.parse(configdata);
          var mqttConfigData = Object.values(configObj);
          var mqttCount = mqttConfigData.length;
          var statusDataObj = JSON.parse(mqtt_status_data_name);
          var mqttStatusData = Object.values(statusDataObj);
          var merged = format.mapMqttDataToDeviceV2([val], mqttrs['data'].length < mqttCount ? Object.fromEntries(mqttStatusData.map((k, i) => [k, mqttData[i]])) : Object.fromEntries(mqttConfigData.map((k, i) => [k, mqttData[i]])))[0];
          var sensorValueData = merged.value_data;
          var value_alarm :number= Number(merged.value_alarm);
          var value_relay :number= Number(merged.value_relay);
          var value_control_relay :number= Number(merged.value_control_relay);
          var sensorValue: any = sensorValueData;
          // ดึงรายละเอียด Alarm จากฟังก์ชันย่อย
          /*
            1	Sensor
            2	Device IO Fan1
            3	Device IO Fan2
          ...
          if(type_id==1){
              var filter:any={};
                      ...
                      filter.sensorValueData = sensorValueData;
                      filter.status_alert = status_alert;
                      filter.status_warning = status_warning; 
                      filter.recovery_warning = recovery_warning; 
                      filter.recovery_alert = recovery_alert; 
                      filter.mqtt_name = mqtt_name;
                      filter.device_name = device_name;
                      filter.action_name = action_name;
                      filter.mqtt_control_on = mqtt_control_on;
                      filter.mqtt_control_off = mqtt_control_off;
                      filter.event = events;
              var getAlarmDetails : any = await this.settingsService.getAlarmDetails(filter);
          }else{
                var filter:any={};
                      filter.alarmTypeId = type_id;   
                      filter.sensorValueData = value_alarm;
                      filter.status_alert = status_alert;
                      filter.status_warning = status_warning; 
                      filter.recovery_warning = recovery_warning; 
                      filter.recovery_alert = recovery_alert; 
          ...
          var alarmStatusSet: number=Number(getAlarmDetails.alarmStatusSet);
          var messageMqttControl: string=getAlarmDetails.messageMqttControl;
          ...
          var date_now = format.getCurrentDatenow();
          var time_now = format.getCurrentTimenow();
          // ตรวจสอบและจัดการ log
           if(type_id==1){
              var dataAlarm: any = sensorValueData;
           }else{
              var dataAlarm: any = value_alarm;
           }
          var setdatachk:any = {
                                alarm_action_id:alarm_action_id,
                                device_id:device_id,
          ...
          var count_alarm: number = Number(await this.settingsService.count_alarmprocesslog(setdatachk));
          var log_alarm_logs: any = await this.settingsService.chk_alarmprocesslog(setdatachk);
          var now_time_cal: number = Number(0);
          var createddate_logs: any = now_time_full;
          if (count_alarm >=1) {
              var createddate_logs: any = log_alarm_logs[0].createddate;
              var log_time = format.timeConvertermas(format.convertTZ(log_alarm_logs[0].createddate, process.env.tzString));
              var now_time_cal: number = Number(format.diffMinutes(now_time_full, log_time));
          }else {
             var now_time_cal: number = Number(0);
          }
          var devicecontrol:any=[];
          if ((alarmStatusSet ==1 || alarmStatusSet ==2) && now_time_cal>=time_life) {
                // Logic for recovery, potentially deleting logs
                var setdata_chk_delete = {
                    alarm_action_id: alarm_action_id,
                    device_id: device_id,
          ...
          var inputCreate:any={};
                  inputCreate.alarm_action_id = alarm_action_id;
                  inputCreate.device_id = device_id;
                  inputCreate.type_id = type_id;
                  inputCreate.event = event; 
                  inputCreate.alarm_type = alarm_type_id; 
                  inputCreate.status_warning = status_warning; 
                  inputCreate.recovery_warning = recovery_warning;
                  inputCreate.status_alert = status_alert;
                  inputCreate.recovery_alert = recovery_alert;
                  inputCreate.email_alarm = 0;
                  inputCreate.line_alarm = 0;
                  inputCreate.telegram_alarm = 0; 
                  inputCreate.sms_alarm = 0;
                  inputCreate.nonc_alarm = 1;
                  inputCreate.date= format.getCurrentDatenow();
                  inputCreate.time= format.getCurrentTimenow();
                  inputCreate.data= dataAlarm;
                  inputCreate.createddate= new Date();
                  inputCreate.updateddate= new Date();
                  inputCreate.data_alarm= dataAlarm;
                  inputCreate.alarm_status= alarmStatusSet;
           var device:any='';
          if (alarmStatusSet == 1 || alarmStatusSet == 2) {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var topic_send: any = encodeURI(mqtt_data_control);
                      var message_send: any = encodeURI(messageMqttControl);
                      var devicecontrol: any = await this.mqttService.devicecontrol(topic_send, message_send);
                      console.log(`2-devicecontrol=>`);
                      console.info(devicecontrol);
                      var now_time_s: any = devicecontrol['dataObject']['timestamp'];
                      var device: any = devicecontrol['dataObject']['device'];
                      var device_status: any = devicecontrol['dataObject']['device_status'];
                      var subject: any = subject + ' Control device';
                      var contents: any = content + ' Control device case-2'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
                          alarm_action_id:alarm_action_id,
                          event:event,
                          device_id:device_id,
                      ...
                      var count_alarm: number = Number(await this.settingsService.count_alarmprocesslog(setdata_chk));
                      if (count_alarm == 0) {
                          await this.settingsService.manageAlarmLog(inputCreate, setdata_chk, validate_count);
                      }
                  } else if (now_time_cal > time_life) {
                      var topic_send: any = encodeURI(mqtt_data_control);
                      var message_send: any = encodeURI(messageMqttControl);
                      var devicecontrol: any = await this.mqttService.devicecontrol(topic_send, message_send);
                      console.log(`2-devicecontrol=>`);
                      ...
                      var device_status: any = devicecontrol['dataObject']['device_status'];
                      var subject: any = subject + ' Control device';
                      var contents: any = content + ' Control device case-1';
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      ...
                      var device_status: any = devicecontrol['dataObject']['device_status'];
                      var subject: any = subject + ' Control device';
                      var contents: any = content + ' Control device case-4';
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
                      ...
                      var device_status: any = devicecontrol['dataObject']['device_status'];
                      var subject: any = subject + ' Control device';
                      var contents: any = content + ' Control device case-3';
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
          ...
          // เพิ่มข้อมูลที่เกี่ยวข้องทั้งหมดลงใน array 
          device_event_control_ar.push({
                ...
                // log_alarm_logs,
                time_life:time_life,
                time_life_cal:time_life+' - '+now_time_cal,
                ...
                subject:subject,
                content:contents,
                data_alarm:inputCreate.data_alarm,
      ...
      // ส่งคืน array ที่มีข้อมูลทั้งหมด
      ...
      for (const [key, val] of Object.entries(device_data_control)) {
          ...
          if(type_id==1){
              var filter:any={};
                      ...
                      filter.mqtt_control_off = mqtt_control_off;
                      filter.event = events;
              var getAlarmDetails : any = await this.settingsService.getAlarmDetailsChkEmail(filter);
          }else{
                var filter:any={};
                      filter.alarmTypeId = type_id;   
                      filter.sensorValueData = value_alarm;
          ...
          var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogemail(setdatachk));
          var log_alarm_logs: any = await this.settingsService.chk_alarmprocesslogemail(setdatachk);
          var now_time_cal: number = Number(0);
          var createddate_logs: any = now_time_full;
          if (count_alarm >=1) {
              var createddate_logs: any = log_alarm_logs[0].createddate;
          ...
          if (alarmStatusSet == 1 || alarmStatusSet == 2) {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send email';
                      var contents: any = content + ' Send email case-2'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
                      ...
                      var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogemail(setdata_chk));
                      if (count_alarm == 0) {
                          var getAlarmDetailsSendEmail : any = await this.settingsService.getAlarmDetailsSendEmail(filter);
                          await this.settingsService.manageAlarmLogEmail(inputCreate, setdata_chk, validate_count);
                      }
                  } else if (now_time_cal > time_life) { 
                      var subject: any = subject + ' Send email';
                      var contents: any = content + ' Send email case-1'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                        let setdata_chk: any = {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send email';
                      var contents: any = content + ' Send email case-4';
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
                      ...
                      var subject: any = subject + ' Send email';
                      var contents: any = content + ' Send email case-3'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      let setdata_chk: any = {
      ...
      for (const [key, val] of Object.entries(device_data_control)) {
          ...
          if(type_id==1){
              var filter:any={};
                      ...
                      filter.mqtt_control_off = mqtt_control_off;
                      filter.event = events;
              var getAlarmDetails : any = await this.settingsService.getAlarmDetailsChkLine(filter);
          }else{
                var filter:any={};
                      filter.alarmTypeId = type_id;   
                      filter.sensorValueData = value_alarm;
          ...
          var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogline(setdatachk));
          var log_alarm_logs: any = await this.settingsService.chk_alarmprocesslogline(setdatachk);
          var now_time_cal: number = Number(0);
          var createddate_logs: any = now_time_full;
          if (count_alarm >=1) {
              var createddate_logs: any = log_alarm_logs[0].createddate;
          ...
          if (alarmStatusSet == 1 || alarmStatusSet == 2) {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send Line';
                      var contents: any = content + ' Send Line case-2'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
                      ...
                      var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogline(setdata_chk));
                      if (count_alarm == 0) {
                          var getAlarmDetailsSendLine : any = await this.settingsService.getAlarmDetailsSendLine(filter);
                          await this.settingsService.manageAlarmLogLine(inputCreate, setdata_chk, validate_count);
                      }
                  } else if (now_time_cal > time_life) { 
                      var subject: any = subject + ' Send Line';
                      var contents: any = content + ' Send Line case-1'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                        var setdata_chk: any = {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send Line';
                      var contents: any = content + ' Send Line case-4';
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
                      ...
                      var subject: any = subject + ' Send Line';
                      var contents: any = content + ' Send Line case-3'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
      ...
      for (const [key, val] of Object.entries(device_data_control)) {
          ...
          if(type_id==1){
              var filter:any={};
                      ...
                      filter.mqtt_control_off = mqtt_control_off;
                      filter.event = events;
              var getAlarmDetails : any = await this.settingsService.getAlarmDetailsChksms(filter);
          }else{
                var filter:any={};
                      filter.alarmTypeId = type_id;   
                      filter.sensorValueData = value_alarm;
          ...
          var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogsms(setdatachk));
          var log_alarm_logs: any = await this.settingsService.chk_alarmprocesslogsms(setdatachk);
          var now_time_cal: number = Number(0);
          var createddate_logs: any = now_time_full;
          if (count_alarm >=1) {
              var createddate_logs: any = log_alarm_logs[0].createddate;
          ...
          if (alarmStatusSet == 1 || alarmStatusSet == 2) {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) { 
                      inputCreate.status = device;
                      var subject: any = subject + ' Send sms';
                      var contents: any = content + ' Send sms case-2';
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
                          alarm_action_id:alarm_action_id,
                      ...
                      var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogsms(setdata_chk));
                      if (count_alarm == 0) {
                          var getAlarmDetailsSendsms : any = await this.settingsService.getAlarmDetailsSendsms(filter);
                          await this.settingsService.manageAlarmLogsms(inputCreate, setdata_chk, validate_count);
                      }
                  } else if (now_time_cal > time_life) { 
                      var subject: any = subject + ' Send sms';
                      var contents: any = content + ' Send sms case-1'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                        var setdata_chk: any = {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send sms';
                      var contents: any = content + ' Send sms case-4'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents; 
                      var setdata_chk: any = {
                      ...
                      var subject: any = subject + ' Send sms';
                      var contents: any = content + ' Send sms case-3'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
      ...
      for (const [key, val] of Object.entries(device_data_control)) {
          ...
          if(type_id==1){
              var filter:any={};
                      ...
                      filter.mqtt_control_off = mqtt_control_off;
                      filter.event = events;
              var getAlarmDetails : any = await this.settingsService.getAlarmDetailsChktelegram(filter);
          }else{
                var filter:any={};
                      filter.alarmTypeId = type_id;   
                      filter.sensorValueData = value_alarm;
          ...
          var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogtelegram(setdatachk));
          var log_alarm_logs: any = await this.settingsService.chk_alarmprocesslogtelegram(setdatachk);
          var now_time_cal: number = Number(0);
          var createddate_logs: any = now_time_full;
          if (count_alarm >=1) {
              var createddate_logs: any = log_alarm_logs[0].createddate;
          ...
          if (alarmStatusSet == 1 || alarmStatusSet == 2) {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send telegram';
                      var contents: any = content + ' Send telegram case-2'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
                      ...
                      var count_alarm: number = Number(await this.settingsService.count_alarmprocesslogtelegram(setdata_chk));
                      if (count_alarm == 0) {
                          var getAlarmDetailsSendtelegram : any = await this.settingsService.getAlarmDetailsSendtelegram(filter);
                          await this.settingsService.manageAlarmLogtelegram(inputCreate, setdata_chk, validate_count);
                      }
                  } else if (now_time_cal > time_life) { 
                      var subject: any = subject + ' Send telegram';
                      var contents: any = content + ' Send telegram case-1'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                        var setdata_chk: any = {
              ...
              if (dataAlarm == 999) {} else {
                  if (count_alarm == 0 && now_time_cal == 0) {
                      var subject: any = subject + ' Send telegram';
                      var contents: any = content + ' Send telegram case-4'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents; 
                      var setdata_chk: any = {
                      ...
                      var subject: any = subject + ' Send telegram';
                      var contents: any = content + ' Send telegram case-3'; 
                      inputCreate.status = device;
                      inputCreate.subject = subject;
                      inputCreate.content = contents;
                      var setdata_chk: any = {
  ...
  //     return await rt;
  // }
  // async manageAlarmLog(inputCreate: any, 
  //                       setdataChk: any, 
  //                       validateCount: number) {
  //     const countAlarm: any = await this.settingsService.count_alarmprocesslog(setdataChk);
  //     if (countAlarm <= validateCount) {
  //         const countCheck: any = await this.settingsService.count_alarmprocesslog({ ...setdataChk,
  //             time: format.getCurrentTimenow()
  //         });
  //         if (countCheck <= 1) {
  //             await this.settingsService.create_alarmprocesslog(inputCreate);
  //             await this.settingsService.create_alarmprocesslogtemp(inputCreate);
  //         }
  //     } else if (countAlarm > validateCount) {
  //         await this.settingsService.delete_alarmprocesslog({
  ...
  //              //  var ResultDatasendEmail: any = await this.settingsService.sendEmail(email, subject,content);
  ...
  //                       setdataChk: any, 
  //                       validateCount: number) {
  //     const countAlarm: any = await this.settingsService.count_alarmprocesslogemail(setdataChk);
  //     if (countAlarm <= validateCount) {
  //         const countCheck: any = await this.settingsService.count_alarmprocesslogemail({ ...setdataChk,
  //             time: format.getCurrentTimenow()
  //         });
  //         if (countCheck <= 1) {
  //             await this.settingsService.create_alarmprocesslogemail(inputCreate);
  //             await this.settingsService.create_alarmprocesslogtemp(inputCreate);
  //         }
  //     } else if (countAlarm > validateCount) {
  //         await this.settingsService.delete_alarmprocesslog_email({
  //             alarm_action_id: setdataChk.alarm_action_id,
  //             device_id: setdataChk.device_id,
  //             type_id: setdataChk.type_id,
  //             date_now: setdataChk.date,
